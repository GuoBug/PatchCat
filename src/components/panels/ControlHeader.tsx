...
export const ControlHeader: React.FC<ControlHeaderProps> = ({
  ...
  const retryAllFailedNodes = useWorkflowStore((s) => s.retryAllFailedNodes);

  const failedNodes = useMemo(() => nodes.filter((n) => n.data.status === 'error'), [nodes]);
  ...
  // Close Quick Add Node menu on click outside (capture phase handles React Flow pane events) or on Escape
  useEffect(() => {
    ...
    // Use capture: true so that canvas/node mousedown stopPropagation does not block closing
          ...
          switch (event.type) {
            ...
            case 'NODE_ERROR':
              ...
              setAlertNotification({
                ...
                failedNodeId: event.payload.nodeId,
              });
              break;
  ...
  const nodePalette: { type: NodeType; label: string; desc: string; color: string }[] = [
    ...
    {
      type: 'condition',
      label: t.nodeTypes.condition,
      desc: t.nodeTypes.conditionDesc,
      color: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30',
    },
    {
      type: 'aggregator',
      label: t.nodeTypes.aggregator,
      desc: t.nodeTypes.aggregatorDesc,
      color: 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/30',
    },
    {
      type: 'http',
    ...
    <>
      <header className="relative z-40 h-14 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-950/90 backdrop-blur-md px-4 flex items-center justify-between shrink-0 select-none shadow-xs transition-colors duration-200">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {/* Multiple Failed Nodes Batch Retry Indicator */}
          {failedNodes.length >= 2 && !isExecuting && (
            <button
              onClick={() => retryAllFailedNodes()}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/15 dark:hover:bg-rose-500/25 border border-rose-300 dark:border-rose-500/30 text-rose-700 dark:text-rose-300 text-xs font-medium transition-all shadow-xs cursor-pointer"
              title={t.ergonomics.retryAllFailed}
            >
              <RotateCcw className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400" />
              <span>{t.ergonomics.failedNodesCount.replace('{count}', String(failedNodes.length))}</span>
              <span className="opacity-40">|</span>
              <span className="font-semibold underline decoration-rose-400/50">{t.ergonomics.retryAllFailed}</span>
            </button>
          )}
              ...
              {alertNotification.cycleNodes && alertNotification.cycleNodes.length > 0 && (
                <div className="pt-1 flex flex-wrap gap-1.5">
                  ...
                  {alertNotification.cycleNodes.map((nid) => (
                    <button
                      key={nid}
                      onClick={() => highlightNode(nid)}
                      className="px-2 py-0.5 rounded bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/20 dark:hover:bg-rose-500/30 border border-rose-200 dark:border-rose-500/40 text-rose-700 dark:text-rose-200 font-mono text-[10px] cursor-pointer transition-colors flex items-center gap-1"
                      title={t.ergonomics.locateNode}
                    >
