      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-[10px] font-mono text-slate-400">
          ...
          {hasOutput && (
            <button
              onClick={handleCopy}
              className="flex items-center gap-1 text-[10px] text-slate-400 hover:text-white px-1.5 py-0.5 rounded bg-slate-800 hover:bg-slate-700 transition-colors"
            >
        ...
        {hasOutput ? (
          <div className="p-2 rounded bg-slate-950/90 border border-pink-500/30 font-mono text-[10px] text-pink-200/90 max-h-24 overflow-y-auto leading-relaxed whitespace-pre-wrap">
            {displayContent}
          </div>
        ) : (
