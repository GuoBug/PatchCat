# 边写边学 AI 工作流引擎（三）：真实踩坑尸检 —— Case #7 的字段冻结陷阱与代码回滚复盘

> **导读**：在上一篇中，我们拆解了由“原输出残片 + 报错路径 + 黄金范式”构成的 L3 自愈状态机。许多人以为自愈机制搭好后便能一劳永逸，但真实的工程研发从来不是一帆风顺的童话。  
> 为了追求极致的 Token 经济性，我们曾设计过一套看似精妙绝伦的“字段级冻结（Field Freezing）”局部微创优化，结果却在极限压测的 **Case #7** 中遭遇滑铁卢，导致大模型陷入死循环。  
> 本篇秉持**干中学（Learning by Doing）与真实求真**的原则，完整复盘这次自作聪明的翻车全过程、底层认知尸检、果断回滚的决策考量，并结合**真实的压测日志与系统 Trace 截图**，深入解析我们构筑的 **L4 优雅降级（Never-Throw Guarantee）** 终极工程安全防线。

---

## 一、引子：一次看似无懈可击的“局部微创手术”
...
如果让大模型重新生成全量 JSON：
1. **白白浪费 Token**：原本推导完美的数百字长文本摘要需要全量重新生成一次；
2. **引发次生风险**：大模型有可能在重写时，把原本已经写对的长文本意外改崩（发生次生语义漂移）。

### 2. “字段级冻结（Field Freezing）”的设想
针对这个痛点，我们与 AI 结对编程推演出了一个近乎完美的优化方案：
...
理论测算显示，这个优化能为重试阶段**节省超过 60% 的 Token 开销**，且理论上 100% 杜绝了正确字段发生漂移的可能。我们兴奋地写了 60 多行局部抽取与合并逻辑，信心满满地推上了测试流水线。

---

## 二、翻车尸检：Case #7 极限压测下的“局部最优解死锁”

现实的巴掌来得极快。当测试流水线跑到第 7 个极限用例（**Case #7：复杂跨字段约束工单**）时，控制台突然大面积飘红，流程陷入了长时间的停滞，最终重试耗尽宣告失败！

### 1. Case #7 的业务契约背景
Case #7 模拟的是售后工单中最严苛的“高危工单深度阐述”与“意图退款联动”场景。在 Zod（L2）层面，我们通过 `superRefine` 构筑了一道跨字段业务守卫：
...
4. **机械合并后的致命冲突**：
   - 模型在缺少全局上下文的情况下随意重写了几个词；
   - 缝合后的对象再次送入 L2 校验，依然无法自洽；
5. **模型彻底抓狂的第 3 轮**：
   - 报错信息又被喂回给模型，但由于 `urgency` 处于冻结盲区，模型无法通过调整优先级来化解冲突；
   - 模型被迫在残缺的上下文里反复横跳，直到重试次数彻底耗尽，测试全线崩溃。

```mermaid
sequenceDiagram
    participant LLM as 大模型
    participant Engine as 工作流引擎 (字段冻结)
    participant Zod as L2 语义校验器
    
    LLM->>Engine: 输出初次结果 (urgency: 5, summary: 短句)
    Engine->>Zod: 执行全局校验
    Zod-->>Engine: 报错: urgency>=4 时 summary 必须 >=15 字
    Note over Engine: 致命操作: 冻结 urgency: 5<br/>仅要求模型修补 summary 字段
    Engine->>LLM: 局部纠错: 请仅调整 summary 字段
    LLM-->>Engine: 修复返回: 缺乏全局语境的局部修补
    Note over Engine: 机械缝合: { urgency: 5, summary: 局部修补 }
    Engine->>Zod: 再次校验缝合数据
    Zod-->>Engine: 致命报错: 跨字段业务契约依旧冲突!
    Note over LLM,Engine: 上下文割裂，模型陷入死循环，3 轮重试全部耗尽崩溃!
```
...
- **方案 B（断舍离回滚）**：承认“字段冻结”的设计假设在本质上违背了大模型的工作原理，彻底删除局部冻结逻辑，回归全局上下文修复。

### 1. 底层原理认知：大模型是“连续语义场”，不是“机械拼图”
在与 AI 搭档反复论证后，我们深刻认识到：
> **核心架构设计原则**：  
> *不要用静态代码的机械拼装，去强行割裂大模型的连续语义场。*

人类写代码习惯于模块化、单点替换（Patching）；但大模型输出 JSON 并非是在填表格，而是依托自回归注意力机制构建的**连续语义场**。字段与字段之间存在着千丝万缕的隐性语义耦合。  
当我们用静态代码强行冻结一部分、修补另一部分时，本质上是在用人类自以为是的确定性逻辑，给大模型制造一个充满断层与矛盾的残缺世界。

### 2. 果断回滚（Rollback）
在工程实践中，**懂得何时回滚比硬撑一个错误的设计更具价值**。  
我们当机立断：**彻底删除全部 60 多行“字段冻结与局部合并”代码，完全回归“原输出全量残片 + 字段诊断 + 黄金范式”的全局修复路线。**
...
从真实的 Trace 日志中可以清晰看到：在保留完整全局上下文后，大模型在第 2 轮精准领会了跨字段要求，将摘要扩充至 17 个字（`"昨天购买的订单一直显示待发货，急需送达，否则将要退款"`），**整个自愈流程在第 2 轮耗时仅 400ms 便完美收敛绿灯通过！**

---

## 四、L4 终极防线：当 3 轮重试全部耗尽，系统如何“绝不崩溃”？

虽然回归全局修复让工作流的自愈率突破了 90%，但作为一个严肃的 **AI 工作流编排（AI Workflow Orchestration）** 引擎，我们必须面对最残酷的底线问题：
...
```
**这在 DAG 状态机中是灾难性的。**  
工作流往往包含多个并发分支与下游待触发节点。一旦核心节点直接未捕获异常抛出，整个运行进程崩溃，所有中间已执行节点的成果丢失，画布上的节点永远卡在 Pending 状态，给终端用户带来极度糟糕的“白屏卡死”体验。

### 2. PatchCat 的 L4 优雅降级（Never-Throw Guarantee）
为了保障工作流流水线的绝对确定性，我们在 L4 层树立了一条硬铁律：**无论遭遇多么恶劣的输出，结构化输出引擎绝不允许向上抛出致命异常导致流程中断。**

当 3 轮重试全部耗尽时，系统执行**确定性优雅降级**，返回一个带有特殊元数据的合规对象：
...
![3轮耗尽优雅降级 Trace](../assets/self-healing-traces/06-max-retries-graceful-degradation.png)

### 4. 实测证据二：物理 Token 严重截断时的瞬时兜底
除了逻辑上的死循环，模型在现实中还会因为 `max_tokens` 设定过小而发生物理截断（只输出了半句 JSON 大括号未闭合）。  
以下为真实捕获的截断日志（截取自 `测试日志及截图/输出被 max_tokens 截断.log`）：
...
面对物理截断，L4 兜底同样保障了引擎**不抛异常、不阻塞下游**，平滑进入降级处理通道：

![截断情况下的 Never-Throw 兜底](../assets/self-healing-traces/04-token-truncated-never-throw.png)
...
1. **警惕“自作聪明的微小优化”**：为了节省几个 Token 而引入破坏模型语义整体性的复杂补丁，往往会引发更不可控的隐性系统死锁；
2. **接受不完美，但掌控确定性**：大模型本质是概率性的，哪怕是 GPT-4o 也无法承诺 100% 的绝对可用。优秀的工程师不应该妄想消灭大模型的所有不确定性，而是要通过 L4 的 **Never-Throw 优雅降级**，让上层业务在面对不确定性时，依然拥有确定性的容错轨道。

### 留白与引线：引擎虽然稳了，代码却变脏了？
当我们把 L1、L2、L3、L4 这套看似坚不可摧的防线跑通后，代码评审（Code Review）中却暴露了一个极其致命的架构缺陷：
> *“为了测试这套工单场景，我们在引擎核心的 `structured-output.ts` 里直接硬编码了工单的特定 Schema 和 130 多行测试用例！如果把这段引擎拿去做法律合同审查、或者做医疗质检工作流，难道还要去改引擎核心代码吗？”*

这是典型的**业务逻辑向系统引擎层下沉污染**。  
我们是如何进行架构清洗与业务彻底解耦的？又是如何通过严格的 **42 样本麦克尼马尔（McNemar）统计检验**，在数学上证明这套系统并非“幸存者偏差”的？

**系列终篇揭秘：《边写边学 AI 工作流引擎（四）：架构纯度清洗与 42 样本统计检验复盘》**。
