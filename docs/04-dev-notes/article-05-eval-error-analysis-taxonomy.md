# 边写边学 AI 工作流引擎（五）：合规率暴涨 23.9%，语义准确率却跌了 7.1%？自愈病理学与双轴归因复盘

> 导读：在上一篇中，我们通过架构清洗把 Schema 业务逻辑彻底从引擎核心剥离，并用 42 组样本完成了配对 McNemar 检验。端到端契约合规率从 69.0% 提升到了 92.9%（+23.9pt）。但在持续分析评测矩阵时，一项反常的数据拉响了警报：意图分类准确率从基线的 85.7% 跌落到了 78.6%（-7.1pt）。为什么自愈机制在修好格式的同时，反而把语义改崩了？本文真实记录我们对评测失败用例的病理学解剖：拆解双轴正交分析框架、深挖 F6 语义掩盖与 F2 示例锁死陷阱，给 AI 系统的自动化评测立下严谨的度量底线。
...
![自愈病理学与双轴归因复盘](../assets/eval-error-analysis-taxonomy-cover.jpg)
...
![双轴正交分析框架与病理图谱](../assets/flowchart-dual-axis-error-taxonomy.svg)
